...
export async function onRequestPost(context){
 ...
 body.users.forEach((raw,index)=>{
  const username=String(raw.username||"").trim(), nom=String(raw.nom||"").trim(), prenom=String(raw.prenom||"").trim(), peloton=String(raw.peloton||"").trim(), promotion=String(raw.promotion||"").trim(), role=normalizeRole(raw.role), lineNumber=Number(raw.lineNumber||index+1);
  if(!nom||nom.length>120) errors.push({lineNumber,username,error:"Le nom est obligatoire et limité à 120 caractères."});
  else if(!/^\d{6}$/.test(username)) errors.push({lineNumber,username,error:"Le NIGEND doit contenir exactement 6 chiffres."});
  else if(!["eleve","cadre"].includes(role)) errors.push({lineNumber,username,error:"Le rôle doit être eleve ou cadre."});
  else if(role==="eleve" && !peloton) errors.push({lineNumber,username,error:"Le peloton est obligatoire pour un élève."});
  else if(seen.has(username)) errors.push({lineNumber,username,error:"Ce NIGEND apparaît plusieurs fois dans le même lot."});
  else { seen.add(username); users.push({username,nom,prenom,peloton,promotion,role,lineNumber}); }
 });
 ...
 try{
  ...
  const exists=await context.env.DB.batch(users.map(u=>context.env.DB.prepare(`SELECT username FROM users WHERE username=? LIMIT 1`).bind(u.username)));
  const existing=[], fresh=[];
  users.forEach((u,i)=>{ if(Array.isArray(exists[i]?.results)&&exists[i].results.length) existing.push({...u,status:"existing"}); else fresh.push(u); });
  const created=[];
  for(const u of fresh){ const temporaryPassword=createTemporaryPassword(), salt=crypto.getRandomValues(new Uint8Array(16)), passwordHash=await hashPassword(temporaryPassword,salt); created.push({...u,temporaryPassword,passwordHash,passwordSalt:bytesToBase64(salt)}); }
  const statements=[];
  for(const u of created) statements.push(context.env.DB.prepare(`INSERT INTO users (username,nom,password_hash,password_salt,active,role,must_change_password,session_version) VALUES (?,?,?,?,1,?,1,1)`).bind(u.username,[u.nom,u.prenom].filter(Boolean).join(" "),u.passwordHash,u.passwordSalt,u.role));
  for(const u of existing) statements.push(context.env.DB.prepare(`UPDATE users SET nom=?, role=?, active=1 WHERE username=?`).bind([u.nom,u.prenom].filter(Boolean).join(" "),u.role,u.username));
  for(const u of users.filter(u=>u.role==="eleve")) statements.push(context.env.DB.prepare(`INSERT INTO discipline_students (nigend,nom,prenom,peloton,promotion,active,updated_at) VALUES (?,?,?,?,?,1,CURRENT_TIMESTAMP) ON CONFLICT(nigend) DO UPDATE SET nom=excluded.nom, prenom=excluded.prenom, peloton=excluded.peloton, promotion=excluded.promotion, active=1, updated_at=CURRENT_TIMESTAMP`).bind(u.username,u.nom,u.prenom,u.peloton,u.promotion));
  if(statements.length) await context.env.DB.batch(statements);
  return jsonResponse({success:true,created:created.map(u=>({username:u.username,nom:[u.nom,u.prenom].filter(Boolean).join(" "),role:u.role,temporaryPassword:u.temporaryPassword,lineNumber:u.lineNumber,status:"created"})),existing,summary:{requested:users.length,created:created.length,existing:existing.length,students:users.filter(u=>u.role==="eleve").length}});
